...
export function ChallengeSidebar({
  ...
  const { isCompleted: checkCompleted, toggleComplete } = useProgress();
  const isCompleted = checkCompleted(projectId);
  const { t } = useLanguage();
          ...
          <div className="space-y-2">
            {hints.map((hint, idx) => (
              ...
              >
                ...
                >
                  <span>
                    {t.challenge.hints.trigger} #{idx + 1}: {hint.title}
                  </span>
                  <svg
                    className={`w-4 h-4 transition-transform ${
                      activeHint === idx ? "rotate-180" : ""
                    }`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
